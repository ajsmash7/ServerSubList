...
def player_list(request):
    ...
    if search_name:
        ...
        return render(request, 'subCityList/player/player_list.html', {'players': players, 'form': form, 'search_name': search_name})

    else:
        message = 'Player Not Found'
        return render(request, 'subCityList/player/player_list.html', {'form': form, 'message': message, 'search_name': search_name})
...
def player_add(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            player = form.save()
            return redirect('subCityList:player_detail', player_pk=player.id)

    else:
        form = PlayerForm()

    return render(request, 'subCityList/player/player_add.html', {'form': form})
...
def player_detail(request, player_pk):
    ...
    return render(request, 'subCityList/player/player_detail.html', {'player': player, 'cities': cities})
...
def player_edit(request, player_pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return render(request, 'subCityList/player/player_edit.html', {'form': form, 'message': message})

    else:
        form = EditPlayerForm(instance=player)
        return render(request, 'subCityList/player/player_edit.html', {'form': form})
